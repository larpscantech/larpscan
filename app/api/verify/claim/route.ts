...
export const POST = withErrorHandler(async (req: Request) => {
  ...
  if (verifyResult?.signals?.transactionHash) {
    await log(runId, `On-chain tx: https://solscan.io/tx/${verifyResult.signals.transactionHash}`);
  } else if (verifyResult?.signals?.transactionAttempted) {
    await log(runId, `Transaction was attempted but not broadcast (insufficient SOL for fees)`);
  }
