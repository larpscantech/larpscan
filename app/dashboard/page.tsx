  ...
  return (
    <div className="flex gap-0.5 p-1 rounded-sm bg-[#0a0a0d] border border-[#1c1c22]">
      {(['contract', 'website'] as const).map((mode) => (
        ...
        >
          {mode === 'contract' ? (
            <><span className="font-mono text-[9px]">◎</span> Mint</>
          ) : (
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // If the URL has ?ca=..., prefill the Solana mint. The debounced active-run
    ...
    if (urlCa && isSolanaMintAddress(urlCa)) {
      setInput(urlCa);
    }
  ...
  const handleSelectRecentScan = useCallback(async (v: import('@/lib/types').RecentVerification) => {
    ...
    const projectStub: DbProject = {
      ...
      logo_url: null, description: null, chain: 'solana', created_at: '',
    };
