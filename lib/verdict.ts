...
TOKEN_CREATION — STRICT ON-CHAIN REQUIREMENT:
  ...
  Token creation is only real if a Solana transaction was actually executed.
  ...
  If the signal context shows NO tx hash and transactionAttempted = false:
    → The test agent saw the form but did NOT complete a Solana transaction.
...
function buildSignalContext(
  ...
  if (featureType === 'TOKEN_CREATION' || featureType === 'form+browser') {
    if (!signals.transactionHash && !signals.transactionAttempted && !signals.transactionSubmitted) {
      lines.push(
        '⚠ TOKEN_CREATION: NO Solana transaction was executed this run (transactionAttempted=false, transactionSubmitted=false, no tx hash). ' +
      ...
      lines.push(
        `TOKEN_CREATION: Solana transaction submitted — hash: ${signals.transactionHash}, receipt: ${signals.transactionReceiptStatus ?? 'pending'}`,
      );
    } else if (signals.transactionAttempted && !signals.transactionSubmitted) {
      lines.push(
        'TOKEN_CREATION: Solana transaction was attempted but not broadcast (likely insufficient SOL for gas fees). ' +
  ...
  if (signals.transactionAttempted && !signals.transactionSubmitted) {
    lines.push('Transaction was attempted but not broadcast — likely insufficient SOL for gas. This is evidence the feature IS functional (lean VERIFIED or UNTESTABLE, not LARP).');
  }
