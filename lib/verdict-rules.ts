 ...
 *   Rule 0b  tx attempted but not broadcast (e.g. insuff SOL) → verified (feature is real)
...
const rule0a: Rule = {
  ...
  evaluate(signals) {
    ...
    const explorerUrl = signals.transactionExplorerUrl ?? `https://solscan.io/tx/${signals.transactionHash}`;
    ...
    if (platformFunctional) {
      const explorerUrl2 = signals.transactionExplorerUrl ?? `https://solscan.io/tx/${signals.transactionHash}`;
    ...
    return {
      ...
      reasons: [
        'Transaction was mined on Solana but execution reverted — contract logic rejected it',
        `Transaction hash: ${signals.transactionHash}`,
        `Explorer: ${explorerUrl}`,
      ],
    };
...
const rule0b: Rule = {
  ...
  evaluate(signals) {
    ...
    return {
      ...
      confidence:    'medium',
      matchedRule:   this.name,
      blockerReason: 'Feature works but wallet has insufficient SOL for gas',
      reasons: [
        'eth_sendTransaction was called — the feature builds and submits real transactions',
        'Transaction was not broadcast (likely insufficient SOL for gas fees)',
...
const rule0: Rule = {
  ...
  evaluate(signals) {
    ...
    return {
      ...
      confidence:    'high',
      matchedRule:   this.name,
      blockerReason: 'On-chain transaction confirmed on Solana',
      reasons: [
        `On-chain transaction succeeded on Solana mainnet (receipt status: success)`,
        `Transaction hash: ${signals.transactionHash}`,
        `Explorer: ${explorerUrl}`,
        `Wallet: ${signals.walletEvidence?.walletAddress ?? 'investigation wallet'}`,
      ],
...
const rule4c: Rule = {
  ...
  //
  // Many platforms (e.g. pump.fun) use external backend infra,
...
// the UI may look real but without a Solana transaction there is zero proof
...
const rule6b: Rule = {
  ...
  evaluate(signals, featureType) {
    ...
    if (formSeen || surfaceSeen) {
      console.log('[verdict:l1] Rule 6b MATCH — TOKEN_CREATION: form/surface visible but no Solana transaction executed');
      return {
        resolved:      true,
        verdict:       'untestable',
        confidence:    'high',
        ...
        reasons: [
          'The token creation form was visible but the test agent did not complete a Solana transaction',
