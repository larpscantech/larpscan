...
import { classifyStepOutcome, deriveWorkflowReachState } from './workflow';
...
export async function dismissConsentBanner(page: Page): Promise<void> {
  ...
  try {
    // Look for a consent modal/banner first — don't auto-click if no overlay
    ...
    for (let i = 0; i < Math.min(count, 3); i++) {
      ...
      console.log(`[executor] Dismissed consent banner: "${(txt ?? '').trim()}"`);
...
export async function executeSteps(
  ...
  while (true) {
    ...
    try {
      // ── Execute action ────────────────────────────────────────────────────
      await performStep(page, effectiveStep, baseDomain);
      stepSucceeded = true;
    } catch (e) {
...
async function performStep(page: Page, step: AgentStep, baseDomain: string): Promise<void> {
  switch (step.action) {

    case 'navigate': {
      const base = baseDomain.replace(/\/$/, '');
      const path = step.path.startsWith('/') ? step.path : `/${step.path}`;
      await page.goto(`${base}${path}`, { waitUntil: 'networkidle', timeout: 12_000 }).catch(() =>
...
export function buildEvidenceSummary(
  ...
  for (const obs of observations) {
    ...
    if (obs.pageText && !obs.isNoop) {
      // Strip JS runtime error noise from per-step text snippets
  ...
  // Final page content
  // Strip JS runtime errors (TypeError, ReferenceError, etc.) from page text
  // before giving it to the verdict LLM. These are common background errors on
  // virtually every React/Next.js app and do NOT mean the feature is broken —
  // they should never drive a FAILED verdict on their own.
  const cleanedFinalText = finalPageState.visibleText
    ...
    .replace(/Cannot read propert(?:y|ies) of (?:undefined|null)[^\n]{0,200}\n?/gi, '')
    .replace(/(?:at\s+\S+\s+\([^)]+\)\n?){2,}/g, '')  // strip stack frames "at foo (bar.js:1:2)"
    .replace(/\n{3,}/g, '\n\n')
    .trim();
  lines.push('\n--- Final Page State ---');
...
export async function handleWalletPopups(
  ...
  if (connectPromptVisible) {
    ...
    if (decision.allowed) {
      ...
      // After React/AppKit hydrates, fire eth_requestAccounts programmatically.
      // This resolves the wagmi/AppKit connection without needing extension popup.
      await page.evaluate(() => {
        ...
        // Also call the force-connect helper to push events into wagmi listeners
        if (typeof w['__chainverifyTriggerConnect'] === 'function') {
          (w['__chainverifyTriggerConnect'] as () => void)();
        }
      }).catch(() => {});
      await page.waitForTimeout(2_000);  // let wagmi/AppKit state update
      ...
      // This button only exists in the Privy modal, not in wagmi/RainbowKit.
      const privyContinueBtn = page
